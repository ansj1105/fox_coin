...
```bash
# 로그인 경로: 301 한 번 후 200이면 정상 (루프 아님)
curl -sI -L -o /dev/null -w "%{http_code}\n" 'https://korion.io.kr/6s9ex74204/grafana/login/'
# Location 헤더만 보기 (최대 5번 따라가기)
curl -sI 'https://korion.io.kr/6s9ex74204/grafana/login/' 2>/dev/null | grep -i location
```

- 301/302가 같은 URL로 반복되면 Nginx `proxy_redirect` 또는 `location = .../grafana/login/` 설정 재확인.
