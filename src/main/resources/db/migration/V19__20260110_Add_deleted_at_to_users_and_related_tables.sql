...
-- 23. referral_relations 테이블에 deleted_at 추가
ALTER TABLE referral_relations ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMP NULL;

COMMENT ON COLUMN referral_relations.deleted_at IS '삭제 시간 (Soft Delete)';
...
CREATE INDEX IF NOT EXISTS idx_referral_relations_deleted_at ON referral_relations(deleted_at);
