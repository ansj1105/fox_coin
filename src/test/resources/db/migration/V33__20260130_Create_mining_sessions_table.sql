...
CREATE INDEX IF NOT EXISTS IDX_mining_sessions_ends_at ON mining_sessions(ends_at);
-- Partial index (WHERE ends_at > NOW()) not supported: NOW() is not IMMUTABLE in predicate
CREATE INDEX IF NOT EXISTS IDX_mining_sessions_user_ends ON mining_sessions(user_id, ends_at);
