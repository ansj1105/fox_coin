-- Test migration: persist swap execution audit fields
ALTER TABLE swaps
...
COMMENT ON COLUMN swaps.fee_rate IS '스왑 실행 시 적용된 수수료 비율';
COMMENT ON COLUMN swaps.fee_amount IS '스왑 실행 시 차감된 수수료 금액';
COMMENT ON COLUMN swaps.spread_rate IS '스왑 실행 시 적용된 스프레드 비율';
